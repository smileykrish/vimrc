...
function! CreateFunctionHeader()
  call LoadArgumentList()
   ...
   let myfile = s:Hdr_SearchFile( "_c_header.txt" )
   if myfile != ""
   ...
   if s:Hdr_SearchText( top_line, bot_line, '\<text\>' ) == 1
      let comment = s:Hdr_ReformatText( comment, col(".") -10, &tw-4 )
...
function! s:Hdr_SearchFile( filename )
   ...
   elseif filereadable( $HOME . "/.vim_config/sources_forked/newheader/" . a:filename )
     return $HOME . "/.vim_config/sources_forked/newheader/" . a:filename
...
function! LoadArgumentList()
  ...
  for s in arg_list
    ...
    let args_desc = split(s, '-')

    let args_list = [] 
    for tok in args_desc
      let args_list = add(args_list, tok)
      continue  " jump to start of loop
      break     " breaks out of a loop
    endfor

    let g:args_desc_list = add(g:args_desc_list, args_list)
    continue  " jump to start of loop
    break     " breaks out of a loop
  endfor
endfunction
